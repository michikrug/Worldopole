...
switch ($request) {
    ...
    case 'pokestop':
        $datas = $manager->getAllPokestops($config->system->only_lured_pokestops);
    ...
    case 'gymshaver':
        ...
        $datas = $manager->getGymShaver($gym_name, $team, $page, $ranking);
    ...
    case 'gymshaver_count':
        $datas = $manager->getGymShaverCount();

        $json = array();
        $json['shavers'] = array_slice($datas[0], 0, 20);
        $json['victims'] = array_slice($datas[1], 0, 20);
        $json['stats'] = $datas[3];
        $locale = array();
        $json['locale'] = $locale;
